#include <roll_test/rviz_q_widget.h>
...
RvizQWidget::RvizQWidget( QWidget* parent )
  : QWidget( parent )
  , linear_velocity_( 0 )
  , angular_velocity_( 0 )
  , linear_scale_( 10 )
...
// skim it.
void RvizQWidget::paintEvent( QPaintEvent* event )
{
...
void RvizQWidget::leaveEvent( QEvent* event )
{
  stop();
}

// The ordinary way to stop: let go of the mouse button.
void RvizQWidget::mouseReleaseEvent( QMouseEvent* event )
{
  stop();
}
...
void RvizQWidget::sendVelocitiesFromMouse( int x, int y, int width, int height )
{  
